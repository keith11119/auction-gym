...
class SurrogateBidder_utree(Bidder):

    def __init__(self, rng, model, utree, input_context = True, output_bid = True, init_gamma=1.0):
        self.prev_gamma = init_gamma
        self.gammas = []
        self.propensities = []
        # self.winrate_model = PyTorchWinRateEstimator()
        ...
        self.output_bid = output_bid
        self.utree = utree
        super(SurrogateBidder_utree, self).__init__(rng)
    ...
    def bid(self, value, context, estimated_CTR):
        ...
        if self.output_bid:
            if self.input_context:
                return float(self.model.predict(context.reshape(-1, len(context)), utree=self.utree))
            else:
                return float(self.model.predict([estimated_CTR, value].reshape(-1, 2), utree=self.utree))
        else:
            if self.input_context:
                gamma = float(self.model.predict(context.reshape(-1, len(context)), utree=self.utree))
            else:
                gamma = float(self.model.predict([estimated_CTR, value].reshape(-1, 2), utree=self.utree))
            bid *= gamma
            self.gammas.append(gamma)
            self.propensities.append(1)
            return bid
