...
class Auction:
    ...
    def simulate_opportunity_complex(self):
        ...
        normal_dist_context = self.rng.normal(0, self.embedding_var, size=self.embedding_size-2)
        skewed_pos_dist_context = skewnorm.rvs(4, size=1, random_state=self.rng)
        skewed_neg_dist_context = skewnorm.rvs(-4, size=1, random_state=self.rng)
        true_context = np.concatenate((normal_dist_context, skewed_pos_dist_context, skewed_neg_dist_context, [1.0]))
